...
export function CollectionSurfer({ items = ITEMS, variant = "magnetic" }: CollectionSurferProps) {
    ...
    const loopedProgress = useTransform(smoothScroll, (value: any ) => value % loopDistance);
    ...
    const x: any = useTransform(loopedProgress, [0, loopDistance], [0, -items.length * stepX]);
...
function Card({
    ...
    return (
        <motion.div
            ref={ref}
            className="absolute w-75 h-100] bg-neutral-900 overflow-hidden shadow-2xl transition-colors duration-500 ease-out group"
            style={{
                transform,
                transformStyle: "preserve-3d",
            }}
